<template>
  ...
  <el-row
    justify="center"
    v-loading="checking_last_step_data"
    element-loading-text="等待验证(上一步)的数据..."
  >
    <el-form :model="sim_info" label-width="100px">
      ...
      <el-form-item v-if="cur_step_num == 2">
        ...
        >
          ...
          <div>
            <span>路网描述: </span>
            <el-input
              v-model="map_text_content"
              type="textarea"
              :autosize="{ minRows: 4, maxRows: 6 }"
            />
          </div>
          <div>
            <!--dev模式下action为localhost, 生产环境下改为ip(例如:127.0.0.1)-->
            <el-upload
              ref="upload_map"
              drag
              :action="uploadUrl"
              ...
              accept=".txt,.xml,.osm"
              :limit="1"
              :before-upload="onBeforeUploadMap"
              :on-success="onUploadMapSuc"
              :on-error="onUploadMapErr"
              :on-exceed="onUploadMapExceed"
            >
            ...
            <el-text
              v-if="verify_map_result !== null"
              class="mx-1"
              :type="verify_map_result ? 'success' : 'danger'"
            >
              {{
                verify_map_result
                  ? " 路网格式验证成功!"
      ...
      <el-form-item v-if="cur_step_num == 3">
        ...
        >
          ...
          <div>
            <el-image
              :src="sim_info.map_pic"
              :preview-src-list="[sim_info.map_pic]"
            >
              <template #error>
                <div class="image-slot">
                  <el-icon><icon-picture /></el-icon>
        ...
        >
          ...
          <div style="display: flex; gap: 20px">
            ...
            <el-form-item label="内置跟驰模型:">
              <el-select
                v-model="sim_info.fixed_od.follow_model"
                placeholder="请选择"
                style="width: 100px"
              >
                <el-option
                  v-for="item in carfollow_list"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                />
            ...
            <el-form-item label="内置换道模型:">
              <el-select
                v-model="sim_info.fixed_od.change_lane_model"
                placeholder="请选择"
                style="width: 100px"
              >
                <el-option
                  v-for="item in changelane_list"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                />
          ...
          <el-tabs type="border-card" v-model="cur_signal_tab">
            <el-tab-pane
              v-for="(tab, index) in signal_tabs"
              :key="tab.id"
              :label="tab.label"
              :name="tab.id"
            >
              <div v-for="(signal, index) in sim_info.fixed_od.sg">
                <div v-if="signal.cross_id == tab.id" style="margin-left: 25px">
                  <!--找到od中对应cross_id的signal-->
                  <div
                    style="
                      display: flex;
                      align-items: center;
                  ...
                  >
                    ...
                    <el-input-number
                      v-model.number="signal.cycle_time"
                      :min="60"
                      :max="300"
                  ...
                  <div style="display: flex; align-items: flex-start">
                    ...
                    <div style="display: flex; flex-direction: column">
                      ...
                      >
                        ...
                        <el-input-number
                          v-model.number="signal.ew_straight"
                          :min="0"
                          :max="signal.cycle_time"
                          controls-position="right"
                          @change="onSignalTimeChanged(signal.cross_id, 'ew_s')"
                        >
                        </el-input-number>
                        <p>&nbsp;&nbsp;秒</p>
                      </div>
                      ...
                      >
                        ...
                        <el-input-number
                          v-model.number="signal.ew_left"
                          :min="0"
                          :max="signal.cycle_time"
                          controls-position="right"
                          @change="onSignalTimeChanged(signal.cross_id, 'ew_l')"
                        >
                        </el-input-number>
                        <p>&nbsp;&nbsp;秒</p>
                      </div>
                  ...
                  <div style="display: flex; align-items: flex-start">
                    ...
                    <div style="display: flex; flex-direction: column">
                      ...
                      >
                        <label style="margin-right: 20px">直行绿灯 </label>
                        <el-input-number
                          v-model.number="signal.sn_straight"
                          :min="0"
                          :max="signal.cycle_time"
                          controls-position="right"
                          @change="onSignalTimeChanged(signal.cross_id, 'sn_s')"
                        >
                        </el-input-number>
                        <p>&nbsp;&nbsp;秒</p>
                      </div>
                      ...
                      >
                        <label style="margin-right: 20px">左转绿灯 </label>
                        <el-input-number
                          v-model.number="signal.sn_left"
                          :min="0"
                          :max="signal.cycle_time"
                          controls-position="right"
                          @change="onSignalTimeChanged(signal.cross_id, 'sn_l')"
                        >
                        </el-input-number>
                        <p>&nbsp;&nbsp;秒</p>
                      </div>
        ...
        >
          ...
          <el-tabs type="border-card" v-model="cur_road_tab">
            <el-tab-pane
              v-for="(tab, index) in road_tabs"
              :key="tab.id"
              :label="tab.label"
              :name="tab.id"
              class="scrollable-road-tab-pane"
            >
              <!--tab.id就是Road_ID, tab.link_id就是LinkID(Object_ID)-->
              <div
                v-for="(flow, index) in sim_info.fixed_od.flows"
                :key="index"
              >
                <div v-if="flow.road_id == tab.link_id" :key="tab.id">
                  <el-divider content-position="left" border-style="dashed"
                    >车辆生成的分布方案与速度(请先设置速度):</el-divider
                  >
                  <div style="display: flex; gap: 10px">
                    <div style="display: flex; align-items: center">
                      ...
                      <el-select v-model="flow.policy" style="width: 100px">
                        <el-option
                          v-for="item in flow_policy_list"
                          :key="item.id"
                          :label="item.name"
                          :value="item.id"
                        />
                    ...
                    >
                      ...
                      <el-input-number
                        v-model.number="flow.extra"
                        controls-position="right"
                        style="width: 65px"
                      ></el-input-number>
                    </div>
                    <div
                      v-if="[0, 1].includes(flow.policy)"
                      style="display: flex; align-items: center"
                    >
                  ...
                  <div v-for="(dist, index) in od.dist" :key="index">
                    <div class="od-row">
                      <p style="margin: 0; padding: 8px">
                        道路{{ tab.id }} -----> 道路{{
                          sim_info.map_json.Link.find(
                            (item: any) => item.Object_ID == dist.dest
                          )?.Road_ID
                        }}
                      </p>
                      <el-input-number
                        v-model="dist.percent"
                        :min="0"
                        :max="1"
                        :precision="2"
                        :step="0.1"
      ...
      <el-form-item v-if="cur_step_num == 4">
        <el-row :gutter="20">
          <el-col
            :span="6"
            v-for="(controlView, index) in control_views"
            :key="index"
            style="
              flex: 1 1 25%;
              max-width: 25%;
            ...
            <el-card>
              <template #header>
                <div class="card-header">
                  ...
                  <el-switch
                    v-model="controlView.use_plugin"
                    inline-prompt
                    style="
              ...
              <div style="margin-top: 20px">
                <el-radio-group v-model="controlView.active_plugin">
                  <el-radio
                    v-for="(plugin_name, index) in controlView.plugins"
                    :key="index"
                    :label="plugin_name"
                    :value="plugin_name"
                    border
                    >{{ controlView.plugin_names[index] }}</el-radio
                  >
                </el-radio-group>
              </div>
            </el-card>
  ...
  <el-row justify="end">
    <el-col :span="6" :offset="18">
      <el-button
        :disabled="btn_last_step_disable"
        type="primary"
        v-if="cur_step_num > 1"
        @click="onBtnChangeStepClicked(-1)"
        >上一步</el-button
      >
      <el-button
        :disabled="btn_next_step_disable"
        type="primary"
        v-if="cur_step_num < max_step_num"
        @click="onBtnChangeStepClicked(1)"
        >下一步</el-button
      >
      <el-button
        :disabled="btn_submit_disable"
        type="primary"
        v-else
        @click="onSubmit"
        >完成</el-button
...
const carfollow_list = [
  { name: "Gipps模型", id: 1 },
  { name: "VDR模型", id: 2 },
  { name: "IDM模型", id: 3 },
];
const changelane_list = [
  { name: "匀加速换道模型", id: 1 },
  { name: "加速度自适应换道模型", id: 2 },
];
const flow_policy_list = [
  { name: "均匀分布", id: 0 },
  { name: "指数分布", id: 1 },
  { name: "伽马分布", id: 2 },
  { name: "正态分布", id: 3 },
...
const control_views = ref<ControlView[]>([
  {
    control_type: "pv",
    plugins: ["test"],
    plugin_names: ["测试"],
...
const onBtnChangeStepClicked = (step: number) => {
  ...
  if (cur_step_num.value == 2) {
  ...
  } else if (cur_step_num.value == 3) {
    //当到了第三步: 设置仿真参数 时, 需要判断第二步是否成功设置了路网地图
    if (verify_map_result.value == false || verify_map_result.value === null) {
      ...
      for (let od of sim_info.value.fixed_od.od) {
        ...
        const road_id = sim_info.value.map_json.Link.find(
          (item: any) => item.Object_ID == orgin_link_id
        )?.Road_ID;
        road_tab_names.push({
          label: "道路" + road_id,
          id: road_id,
          link_id: orgin_link_id,
        });
      }
      ...
      for (let controller_value of sim_info.value.map_json.Controller) {
        ...
        signal_tab_names.push({
          label: "红绿灯" + (cross_id + 1),
          id: cross_id,
        });
      }
      signal_tabs.value = signal_tab_names;
      if (signal_tab_names.length > 0)
        cur_signal_tab.value = signal_tab_names[0].id;
    }
  } else if (cur_step_num.value == 4) {
    //当到了第4步时, 需要判断第三步的OD数据是否正确
    checking_last_step_data.value = true;
    ...
    for (let signal_value of sim_info.value.fixed_od.sg) {
      let time_all =
        signal_value.ew_left +
        signal_value.ew_straight +
        signal_value.sn_left +
        signal_value.sn_straight;
  ...
  if (response.res != "ERR_OK") {
    ...
    GetMapJson().then((map_json_obj) => {
      ...
      for (let link_value of sim_info.value.map_json.Link) {
        if (
          link_value.Is_Origin == 1 &&
          link_value.Link_Start.Object_Type == "M"
        ) {
      ...
      for (let controller_value of map_json_obj.Controller) {
        //通过Controller设置FixedOD中的SG->signal数据
        let one_signal: signal = {
          cross_id: 0,
          cycle_time: 0,
          ew_left: 0,
          ew_straight: 0,
...
function onSignalTimeChanged(cross_id: number, type: string) {
  if (auto_update_signal.value == true) {
    ...
    if (type == "cycle") {
      ...
      target_signal.cycle_time =
        target_signal.sn_left +
        target_signal.sn_straight +
        target_signal.ew_left +
        target_signal.ew_straight;
    }
  }
}

//当输入框内OD percent改变时
function onOdPercentChanged(
  orgin_id: number,
  dest_id: number,
  changed_per: number
) {
  if (auto_update_od.value == true) {
    for (let i = 0; i < sim_info.value.fixed_od.od.length; i++) {
      ...
      if (cur_od_value.orgin_id == orgin_id) {
        ...
        const new_others_per: number =
          residual_per / (cur_od_value.dist.length - 1);
